...
describe("Circle Integration Test", () => {
  ...
  describe("Contract Setup", () => {
    describe("Ethereum Goerli Testnet", () => {
      ...
      it("Should Set Source Relayer Fee for USDC", async () => {
        // set the relayer fee for USDC on eth
        const receipt = await ethCircleRelayer
          .updateRelayerFee(CHAIN_ID_ETH, ethUsdc.address, ethRelayerFee)
          .then((tx: ethers.ContractTransaction) => tx.wait())
          .catch((msg: any) => {
            // should not happen
            console.log(msg);
    ...
    describe("Avalanche Fuji Testnet", () => {
      ...
      it("Should Set Source Relayer Fee for USDC", async () => {
        // set the relayer fee for USDC on avax
        const receipt = await avaxCircleRelayer
          .updateRelayerFee(CHAIN_ID_AVAX, avaxUsdc.address, avaxRelayerFee)
          .then((tx: ethers.ContractTransaction) => tx.wait())
          .catch((msg: any) => {
            // should not happen
            console.log(msg);
    ...
    describe("Transfer Tokens With Relay Logic", () => {
      ...
      it("Should Transfer Tokens With Relay On Ethereum (Out of Sync Relayer Fees)", async () => {
        // create transferTokenWithRelay parameters
        const targetRecipientWallet =
          "0x" + tryNativeToHexString(avaxWallet.address, "avalanche");
        ...
        // Set the relayer fee for the target chain to zero, this will test
        // how the contract handles fees when relayer fees are out of sync
        // on Eth and Avax.
        {
          const receipt = await ethCircleRelayer
            .updateRelayerFee(CHAIN_ID_AVAX, ethUsdc.address, "0")
            .then(async (tx) => {
              const receipt = await tx.wait();
              return receipt;
            })
      ...
      it("Should Redeem Tokens With Relay On Avalanche (Out of Sync Relayer Fees)", async () => {
        // create RedeemParameters struct to invoke the target contract with
        const redeemParameters: RedeemParameters = {
          circleBridgeMessage: localVariables.circleBridgeMessage!,
          circleAttestation: localVariables.circleAttestation!,
        ...
        // Fetch the relayer fee for USDC on both chains to confirm that they
        // are out of sync. We set the relayer fee on ethereum to zero in
        // the previous test.
        const relayerFeeOnEth = await ethCircleRelayer.relayerFee(
          CHAIN_ID_AVAX,
          ethUsdc.address
        );
        const relayerFeeOnAvax = await avaxCircleRelayer.relayerFee(
          CHAIN_ID_AVAX,
          avaxUsdc.address
        );
        expect(relayerFeeOnEth.eq(relayerFeeOnAvax)).is.not.true;
        ...
        // validate recipient token balance
        expect(
          balanceAfter
            .sub(balanceBefore)
            .eq(amountFromEth.sub(toNativeTokenAmountEth))
        ).is.true;
