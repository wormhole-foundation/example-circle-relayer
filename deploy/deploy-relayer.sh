...
if [[ "$1" == "--testnet" ]]; then
    ...
    kubetpl render ./circle-relayer.deployment.yaml -s AWS_ACCOUNT="$AWS_ACCOUNT_STAGING" -s TAG="$2" | kubectl apply -f -
    ...
    kubetpl render ./circle-relayer.deployment.yaml -s AWS_ACCOUNT="$AWS_ACCOUNT_PRODUCTION" -s TAG="$2" | kubectl apply -f -
    kubectl apply -f circle-relayer.service.yaml
else
