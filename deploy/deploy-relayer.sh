...
if [[ "$1" == "--testnet" ]]; then
    ...
    kubectl apply -f testnet/circle.configmap.yaml
    kubetpl render ./circle.deployment.yaml -i testnet/staging.env -s TAG="$2" | kubectl apply -f -
    kubectl apply -f circle-relayer.service.yaml
elif [[ "$1" == "--mainnet" ]]; then
    source mainnet/env.mainnet.sh
    kubectl apply -f mainnet/tbr.configmap.yaml
    kubetpl render ./circle.deployment.yaml -i mainnet/production.env -s TAG="$2" | kubectl apply -f -
    kubectl apply -f circle-relayer.service.yaml
else
